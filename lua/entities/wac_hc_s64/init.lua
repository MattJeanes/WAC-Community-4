...
function ENT:AddStuff()
	...
	self:AddOnRemove(e1)
	self.WinchL=10
	self.Winch=constraint.Rope(self, e1, 0, 0, Vector(-20,0,102), Vector(0,0,0), self.WinchL, 0, 40000, 3, "cable/cable2", false)
	self.Grabber=e1
end
...
function ENT:ChangeRopeLength(len)
	if len and IsValid(self.Grabber) and IsValid(self.Winch) then
		self.Winch:Remove()
		self.WinchL=len
		self.Winch=constraint.Rope(self, self.Grabber, 0, 0, Vector(-20,0,102), Vector(0,0,0), self.WinchL, 0, 40000, 3, "cable/cable2", false)
	end
end
